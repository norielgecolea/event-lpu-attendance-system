...
public class TapErrorLogService {
    ...
    public TapErrorLogResponse record(String identifier, Long eventId, String eventTitle, String location) {
        TapErrorLog entry = new TapErrorLog();
        entry.setIdentifier(identifier == null ? "" : identifier.trim());
        entry.setEventId(eventId);
        entry.setEventTitle(eventTitle);
        entry.setLocation(location);
        entry.setTappedAt(Instant.now());
        tapErrorLogRepository.persist(entry);
        log.info(
                "Recorded tap error identifier={} eventId={} location={}",
                entry.getIdentifier(),
                entry.getEventId(),
                entry.getLocation()
        );
